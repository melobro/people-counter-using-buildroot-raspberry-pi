...
#define LED_PIN_NUM_D 25  // GPIO 25 for the LED
...
while (1) {
    ...
    if (count > 1) {set_value(1, LED_PIN_NUM_D);}  // Turn the LED on
    else {set_value(0, LED_PIN_NUM_D);} 
...
void setup_sensors() {
    ...
    set_direction(OUT, LED_PIN_NUM_D);  // Set the LED pin as output
}
