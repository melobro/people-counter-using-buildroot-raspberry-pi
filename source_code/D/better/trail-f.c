...
#define OPTIMAL_DISTANCE_THRESHOLD 10 // Threshold distance in cm
#define OPTIMAL_DURATION_THRESHOLD 1000000 // 1 second in microseconds
...
int main() {
    ...
    while (1) {
        ...
        if (ultrasonic_active) {
            ...
            if (dist <= OPTIMAL_DISTANCE_THRESHOLD) {
                if (!person_in_range) {
                    ...
                    if (elapsed_time >= OPTIMAL_DURATION_THRESHOLD) {
                        printf("Person in optimal distance for more than 1 second. Capturing video...\n");
                        capture_video();
                        person_in_range = 0; // Reset person_in_range flag
                    }
                }
            } else {
                person_in_range = 0; // Reset person_in_range flag if person moves out of range
            }
        }
